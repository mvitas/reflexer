...
public class RXDatabaseHelper extends SQLiteOpenHelper {
	...
	public static String COLUMN_RX_STIMULI_PROPERTY_VALUE = "rx_property_value";
...
//			db.setForeignKeyConstraintsEnabled(true);
	...
	public int insertRxReactionProperty(SQLiteDatabase db, RXReactionProperty property, int reactionId){
		...
		cv.put(COLUMN_RX_STIUMLI_ID, reactionId);
		
		return (int) db.insertOrThrow(TABLE_RXPROPERTY, null, cv);
	}

	public int insertRxStimuliProperty(SQLiteDatabase db, RXStimuliProperty property, int stimuliId){
		ContentValues cv = property.toContentValues();
	...
	public int insertRxReaction(SQLiteDatabase db, RXReaction reaction){
		return (int) db.insertOrThrow(TABLE_RX_STIMULI, null, reaction.toContentValues());
	}
	...
	public int insertRxReflex(SQLiteDatabase db, RXReflex reflex){
		...
		try {
			...
			//insert stimuli and then when we have the ID, insert the stimuli properties which depend on stimuli 
			...
			//insert reaction and then when we have the ID, insert the reaction properties which depend on reaction
	...
	// tu bi trebalo sve lijepo se brisati sa foreign key constraintima - izvuc bazu iz emulatora i probat
	...
	public int deleteRxStimuliProperty(SQLiteDatabase db, int id){
		return db.delete(TABLE_RX_STIMULI_PROPERTY, COLUMN_RX_STIMULI_PROPERTY_ID + " = ?", new String []{String.valueOf(id)});
	}
	...
	public int updateRxReactionProperty(SQLiteDatabase db, RXReactionProperty property){
		return db.update(TABLE_RXPROPERTY, property.toContentValues(), COLUMN_RXPROPERTY_ID + " = ?", new String []{String.valueOf(property.getId())});
	}
	// ako je id -1 onda insertaj -- znaci treba mi jos id Stimulija --> ovu logiku cu raditi u updateStimuli
	// ako nije -1 onda samo updateaj vrijednosti
	public int updateRxStimuliProperty(SQLiteDatabase db, RXStimuliProperty property){
		return db.update(TABLE_RX_STIMULI_PROPERTY, property.toContentValues(), COLUMN_RX_STIMULI_PROPERTY_ID + " = ?", new String []{String.valueOf(property.getId())});
	}
	
	public int updateRxReaction(SQLiteDatabase db, RXReaction reaction){
		return db.update(TABLE_RXREACTION, reaction.toContentValues(), COLUMN_REACTION_ID + " = ?", new String []{String.valueOf(reaction.getId())});
	}
	
	public int updateRxStimuli(SQLiteDatabase db, RXStimuli stimuli){
		return db.update(TABLE_RX_STIMULI, stimuli.toContentValues(), COLUMN_STIMULUS_ID + " = ?", new String []{String.valueOf(stimuli.getId())});
	}
	...
	public boolean updateRxReflex(SQLiteDatabase db, RXReflex reflex){
		...
		try {
			...
			for (RXStimuliProperty sp : stimuli.getParamsList()){
				if (sp.getId() == NEW_ITEM){ //this is a newly added property and therefore it has to be inserted
			...
			if (db.update(TABLE_RX_REFLEX, reflex.toContentValues(), COLUMN_RX_REFLEX_ID + " = ?", new String []{String.valueOf(reflex.getId())}) == 1){
				reflexUpdated = true;
			}			
